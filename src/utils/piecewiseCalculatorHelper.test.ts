...
import {
  ...
  getResult,
  indexValue,
} from "./piecewiseCalculatorHelpers";
...
describe("getResult", () => {
  ...
  test("returnerar categoryLower om concentration is exakt concentrationLower", () => {
    const result = getResult(data, 35.5);
    expect(Math.round(result)).toBe(101);
  });

  test("returnerar categoryUpper om contration is exakt concentrationUpper", () => {
    const result = getResult(data, 35.4);
...
describe("indexValue", () => {
  test("indexValue finds rätt interval baserat on concentration", () => {
    const index = indexValue(data, 35);
    expect(index?.category).toBe("Moderate");
  });

  test("indexValue returnerar undefined if no match hittas", () => {
    const index = indexValue(data, -10);
