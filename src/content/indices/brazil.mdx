...
Brazil’s SMAC IQA calculation consists of five categories: 

<ColorScale index="brazil" value={true} variations="Rio de Janeiro"/>

_Note_: Adapted from https://jeap.rio.rj.gov.br/je-metinfosmac/institucional/docs/entenda_iqa.pdf


Brazil’s CETESB  ( São Paulo )IQA calculation consists of five categories with different ranges: 

<ColorScale index="brazil" value={true} variations="São Paulo"/>
...
1. **Rio De Janeiro - SMAC calculation process**
Identify Pollutants: First, measure the concentrations of the relevant pollutants SO2 (ug/m3), CO (ppm), PM10 (ug/m3), O3 (ug/m3), NO2 (ug/m3). For each pollutant, the breakpoint table 
is used to establish thresholds that categorize the levels of pollution into the 5 different categories. 


<BreakpointsTable index="brazil" />

2. **Determine Concentration Ranges** 
...
4. The overall IQA value is the highest pollutant sub-index value calculated from the individual pollutant concentration. This value indicates the overall air quality.

5. Classification: The AQI value is then classified into categories based on predetermined ranges.
...
They only mention that specific air pollutants, SO2, CO, PM10, O3, and NO2 were monitored, but doesn’t detail requirements 
...
**Sao Paulo - CETESB calculation process**
1. In São Paulo, the Air Quality Index (AQI) is calculated using the combined concentrations of six primary pollutants: PM2.5, PM10, O3, NO2, SO2, and CO. The AQI 
...
times. For example, ozone levels are measured as an 8-hour mean, while PM10 is evaluated over a 24-hour period. The AQI reflects health implications, indicating when 
...
is independent of any interim targets or local standards and focuses solely on pollutant concentrations and their associated health risks.


2. Determine Concentration Ranges: Each pollutant included in the IQA calculation is associated with defined concentration intervals that correspond to specific air quality 
...
3. Use the Formula: For each pollutant, the corresponding IQA sub-index is calculated using a standardized piecewise linear formula. This formula interpolates the pollutant concentration 
...
the overall IQA. While it specifics that the pollutants, SO2, CO, PM10, O3, and NO2 are monitored, 
