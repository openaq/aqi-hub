...
The Índice de Qualidade do Ar (IQAr) is the air quality index used by the state of Espírito Santo, Brazil. It provides a standardized numerical indicator of air quality conditions 
measured at monitoring stations and is used in official reporting across the state’s automatic air monitoring networks (RAMQAr). The concentration thresholds used in the index are based 
on reference values from the World Health Organization (2021) air quality guidelines and Brazil’s national air quality standards established under CONAMA Resolution No. 506/2024. The 
index is currently reported for monitoring stations in the RAMQAr Greater Vitória and RAMQAr South networks, serving the municipalities of Anchieta and Guarapari.
...
Brazil’s IEMA (Instituto Estadual de Meio Ambiente e Recursos Hídricos) IQAr calculation also consists of five categories with the following ranges: 

<ColorScale variant="Espírito Santo" index="brazil" value={true} />

_Note_: Adapted from https://iema.es.gov.br/qualidadedoar/indicedequalidadedoar 
...
In São Paulo, the Air Quality Index (AQI) is calculated using the concentrations of six primary pollutants: PM<sub>2.5</sub>, PM<sub>10</sub>, O<sub>3</sub>, NO<sub>2</sub>, SO<sub>2</sub>, and CO. The AQI 
...
The IQAr is a dimensionless numerical index ranging from 0 to 400. The Air Quality Index is calculated using the combined concentrations of six primary pollutants: PM<sub>10</sub>, PM<sub>2.5</sub>, O<sub>3</sub>, CO, NO<sub>2</sub>, and SO<sub>2</sub>. 

<BreakpointsTable variant="Espírito Santo" index="brazil" />

For each pollutant, a sub-index value is calculated using piecewise linear functions across five concentration ranges. The final IQAr reported for a monitoring station is the highest sub-index value among all measured pollutants. 

This calculation and reporting methodology follows the technical guidance established in the Guia Tecnico de Qualidade do Ar issued by Brazil’s Ministry of the Environment (2025). 
...
"Guia Da Qualidade Do Ar." Governo Do Estado Do Espírito Santo, [iema.es.gov.br/Media/iema/CQAI/Cartilha/Guia_Qualidade_Ar_ebook.pdf](https://iema.es.gov.br/Media/iema/CQAI/Cartilha/Guia_Qualidade_Ar_ebook.pdf)

“IEMA - Qualidade Do Ar.” Iema.es.gov.br, [iema.es.gov.br/qualidadedoar/indicedequalidadedoar](iema.es.gov.br/qualidadedoar/indicedequalidadedoar).
