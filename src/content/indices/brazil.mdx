...
The IQAr is a dimensionless numerical index ranging from 0 to 400, calculated using piecewise linear functions across five concentration ranges 
for each pollutant. The index is computed individually for each pollutant using the measured concentration and the defined concentration breakpoints 
for that pollutant 

<BreakpointsTable variant="Espírito Santo" index="brazil" />

For each pollutant, a sub-index value is calculated. The final IQAr reported for a monitoring station is the highest sub-index value among all measured pollutants. 
