...
The BelAQI (Belgian Air Quality Index) was developed in the context of the European ‘JOAQUIN’ project. It is an adaptation of 
...
It focuses on health impacts related to short-term exposure to pollutants like PM2.5, NO2, and O3. 
...
data from several pollutants into a single number on a scale from 1 (excellent) to 10 (extremely bad). This index consists of four pollutants: 
an 8-hour average of Ozone (O3), an hourly maximum of Nitrogen Dioxide (NO2), a 24-hour average of Sulfur Dioxide (SO2), and a 24-hour average of 
...
<ColorScale index="belgiumI" value={false} />
...
<Admonition type="note">
  ...
  into 10 discrete levels, ranging from Excellent (1) to Extremely 
  Bad (10). Each category corresponds to concentration thresholds for 
...
The old BelAQI, before November 2022 was based on 2005 WHO air quality guidelines. It used 24-hour averages for PM2.5 and PM10. Mortality risk was calculated on 
Relative Risk (RR) values from WHO’s HRAPIE report: 


- PM2.5: 1.0123 → 1.23% increase in daily mortality per 10 µg/m³
- O₃: 1.0029 → 0.29%
- NO₂: 1.0027 → 0.27%

**Old BelAQI:** 

<BreakpointsTable index="belgiumI" />

_Note_: https://www.irceline.be/en/air-quality/measurements/belaqi-air-quality-index/information
...
- Lower limits for PM2.5, PM10, and NO₂.
- New interim targets for pollutants.
- A shift toward using hourly averages for real-time tracking.    



**New BelAQI Daily Index Table:** 

<BreakpointsTable index="belgiumII" />
...
**Example (PM2.5):**
If daily "Poor" = 16–25 µg/m³
→ Hourly "Poor" = 21–35 µg/m³ (25 × 1.51 ≈ 38)

(The result is rounded to the nearest whole number to make the index easier to communicate and implement. In this case 37.75 → 38 µg/m³)


**New BelAQI Hourly Index Table:** 

<BreakpointsTable index="belgiumIII" />

1. Each pollutant’s concentration is mapped to an index value between 1 (Excellent) and 10 (Horrible) using pre-defined breakpoint thresholds (which differ for daily vs. hourly values).
2. The final BelAQI is simply the maximum of the sub-indices. 
...
<BreakpointsTable index="belgiumIV" />

1. For each pollutant, a sub-index value is determined by matching the measured concentration to the corresponding range in the table
    a. This is a direct categorical assignment – for example, if the 8-hour average ozone (O3) level is 68 ug/m3, it falls in the 61-80 range, which corresponds to an index value of 4 (Fairly Good) 
    b. This process is repeated for all four pollutants (O3, NO2, SO2, PM10).

2. The final BelATMO index is the maximum of these four sub-indices (i.e., the worst pollutant level).

- Geographic Range: This index is calculated for the country, each of the three Regions, major urban areas, and some districts. 
- Note: The BelATMO is qualitative and meant for public communication only, providing a simplified view of air quality. It does not involve any interpolation or averaging across pollutants. This index should not be used for scientific studies or regulatory decisions — for those, raw pollutant concentration data must be used.
