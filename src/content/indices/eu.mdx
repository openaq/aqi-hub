...
<BreakpointsTable variant="old" index="eu" />
...
Air quality categories are now based on equal relative risk of short-term mortality from air pollution exposure. The system uses PM<sub>2.5</sub> as the reference pollutant, with its concentration thresholds defined using WHO guideline levels and interim targets. Thresholds for other pollutants are then calculated so that each band corresponds to a similar level of health risk.
...
<BreakpointsTable variant="new" index="eu" />
